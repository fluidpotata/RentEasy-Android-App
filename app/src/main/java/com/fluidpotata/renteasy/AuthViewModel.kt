...
class AuthViewModel(
    ...
    fun loadMyBills(onResult: (Result<BillsGetResponse>) -> Unit) {
        viewModelScope.launch {
            try {
                val resp = repository.getMyBills()
                onResult(Result.success(resp))
            } catch (e: Exception) {
                onResult(Result.failure(e))
            }
    ...
    fun payBill(bill: Int, tID: String, onResult: (Result<PayBillResponse>) -> Unit) {
        viewModelScope.launch {
            try {
                val resp = repository.payBill(bill, tID)
                onResult(Result.success(resp))
            } catch (e: Exception) {
                onResult(Result.failure(e))
            }
    ...
    fun loadUnverifiedBills(onResult: (Result<BillsGetResponse>) -> Unit) {
        viewModelScope.launch {
            try {
                val resp = repository.getUnverifiedBills()
                onResult(Result.success(resp))
            } catch (e: Exception) {
                onResult(Result.failure(e))
            }
    ...
    fun verifyBill(paymentId: Int, onResult: (Result<GenerateBillResponse>) -> Unit) {
        viewModelScope.launch {
            try {
                val resp = repository.verifyBill(paymentId)
                onResult(Result.success(resp))
            } catch (e: Exception) {
                onResult(Result.failure(e))
            }
